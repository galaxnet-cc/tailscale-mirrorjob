...
pattern_package_version = re.compile(r"^Version: (.+)$", re.MULTILINE)
...
def compare_versions(version1, version2):
    ...
    # 确保版本号不为空，并移除版本号中的波浪线及其后的部分
    clean_version1 = version1.split('~')[0] if version1 else '0'
    clean_version2 = version2.split('~')[0] if version2 else '0'
    
    # 如果处理后的版本号为空字符串，则将其视为"0"
    clean_version1 = clean_version1 if clean_version1.strip() else '0'
    clean_version2 = clean_version2 if clean_version2.strip() else '0'
    
    # 将版本号字符串分割成数字列表
    v1 = tuple(map(int, clean_version1.split('.')))
    v2 = tuple(map(int, clean_version2.split('.')))
...
def apt_mirror(base_url: str, dist: str, repo: str, arch: str, dest_base_dir: Path, min_version: str, deb_set: Dict[str, int])->int:
    ...
    for pkg in pkgidx_content.split('\n\n'):
        ...
        try:
            ...
            pkg_version = pattern_package_version.search(pkg).group(1)
...
def main():
    ...
    parser.add_argument("--min_version", type=str, help="e.g. 1.4.2")
